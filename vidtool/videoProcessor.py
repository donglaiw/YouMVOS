...
class videoProcessor(object):
    ...
    def downloadVideo(self, video_url, video_folder):
        video_file = os.path.join(video_folder, video_url)
        if not os.path.exists(video_file):
            vutil.mkdir(video_file)
    ...
    def extractFrames(self, video_file, frame_folder, frame_fmt):
        frame_file = os.path.join(frame_folder, frame_fmt % 1) # index start from 1
        if not os.path.exists(frame_file):
            vutil.mkdir(frame_folder)
            os.system(self.data.LIB_FFMPEG + ' -i %s %s' % (video_file, os.path.join(frame_folder, frame_fmt)))
        else:
    ...
    def shotDetection(self, frame_template, result_folder, thres_dark = 50, thres_diff = 20, thres_shot_len = 1):
        output_file = os.path.join(result_folder, 'shot.txt')
        if not os.path.exists(output_file):
            self.lib_shot_detection.setFolder(frame_template, result_folder)
            # compute rgb diff
